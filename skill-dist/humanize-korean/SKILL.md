...
**모델 무결성 및 자동 위임 (Model Integrity & Auto-Invoke):**
- 모든 윤문 및 탐지 작업은 미묘한 어감 포착을 위해 반드시 **Pro급 모델**(`model: pro`)에서 수행되어야 한다.
- 메인 세션이 Flash 모델일 경우, 직접 작업을 수행하지 않고 반드시 **`invoke_agent`** 도구를 사용하여 독립된 Pro 세션을 생성하고 업무를 위임한다.
- 위임받은 하위 에이전트는 단순히 텍스트로만 반환하지 않고, 반드시 **`write_file`** 도구를 사용하여 지정된 워크스페이스(`_workspace/[run_id]/`)에 `final.md`와 `summary.md`를 물리적으로 생성해야 한다.
